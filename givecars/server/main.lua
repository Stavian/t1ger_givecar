...
function givevehicle(_source, _args, vehicleType)
	if havePermission(_source) then
		...
		elseif _args[3] ~= nil then
			...
			TriggerClientEvent('esx_giveownedcar:spawnVehiclePlate', _source, _args[1], _args[2], plate, playerName, 'player', vehicleType, _args[3])
		else
			local playerName = GetPlayerName(_args[1])
			TriggerClientEvent('esx_giveownedcar:spawnVehicle', _source, _args[1], _args[2], playerName, 'player', vehicleType, _args[3])
		end
	else
...
function _givevehicle(_source, _args, vehicleType)
	if _source == 0 then
		...
		elseif _args[3] ~= nil then
			...
			plate = string.upper(plate)
			TriggerClientEvent('esx_giveownedcar:spawnVehiclePlate', sourceID, _args[1], _args[2], plate, playerName, 'console', vehicleType, _args[3])
		else
			local playerName = GetPlayerName(_args[1])
			TriggerClientEvent('esx_giveownedcar:spawnVehicle', sourceID, _args[1], _args[2], playerName, 'console', vehicleType, _args[3])
		end
	end
end
...
AddEventHandler('esx_giveownedcar:setVehicle', function (vehicleProps, playerID, vehicleType, name)
	local _source = playerID
	local xPlayer = ESX.GetPlayerFromId(_source)

		if vehicleType == "aircraft" then

			MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle, stored, type, model, name, garage) VALUES (@owner, @plate, @vehicle, @stored, @type, @model, @name, @garage)',
	{
		...
		if Config.ReceiveMsg then
			...
			MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle, stored, type, model, name) VALUES (@owner, @plate, @vehicle, @stored, @type, @model, @name)',
	{
		['@owner']   = xPlayer.identifier,
		['@plate']   = vehicleProps.plate,
		['@vehicle'] = json.encode(vehicleProps),
